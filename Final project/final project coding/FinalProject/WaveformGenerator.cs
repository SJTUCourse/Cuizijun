...
using Automation.BDaq;
    ...
    class WaveformGenerator
    {
       ...
       public uint m_pointPerPeriod;
        ...
        public double GetOnePoint(WaveformStyle style, int index, Double amplitude ,Double offset)
        {
        ...
        public void GetOnePeriod(Double[] waveformBuffer,WaveformStyle style, Double amplitude, Double offset)
        {
            for (int i = 0; i < m_pointPerPeriod; i++ )
            {
                waveformBuffer[i] = GetOnePoint(style, i, amplitude,offset);
            }
        }

        public void GetMultipleWaveform(Double[] waveform,int waveCount, WaveformStyle[] styles,Double[] amplitude,Double[] offset)
        {
            for (int i = 0; i < m_pointPerPeriod;i++ )
            {
                for (int j = 0; j < waveCount; j++)
                {
                    waveform[j + i * waveCount] = GetOnePoint(styles[j], i, amplitude[j], offset[j]);
                }
            }
        }
        #endregion
