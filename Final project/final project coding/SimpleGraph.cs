...
class SimpleGraph
{
   ...
   Double m_shiftCount;
    float m_XcordChangeRate;
    int m_AmplificationFactor;
   ...
   public Double XCordTimeOffset
   {
    ...
    public float  XcordChangeRate
    {
        get
        {
            return m_XcordChangeRate;
        }
        set
        {
            m_XcordChangeRate = value;
        }
    }
      ...
      for (int index = 0; index < drawPoint; ++index)
      {
         for (int i = 0; i < m_plotCount; i++)
         {
            m_dataPointBuffer[i][index].Y = (float)Math.Ceiling(YCordDividedRate *
               (m_YCordRangeMax - m_drawDataBuffer[(int)(m_plotCount * (index + m_mapDataIndexPerPlot + m_shiftCount) + i)]*m_AmplificationFactor));
            m_dataPointBuffer[i][index].X = (float)Math.Round(1.5*(index) * m_XCordDividedRate * m_XcordChangeRate);
         }
      }
