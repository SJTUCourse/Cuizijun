...
class SimpleGraph
{
   ...
   public SimpleGraph(Size bmSize, Control control)
   {
    ...
    #region properties
    public Double m_AmplificationFactor
    {
        set
        {
   ...
   public Double m_XcordChangeRate
    {
        set
        {
            XcordChangeRate = value;
        }
    }
   ...
   private void Draw()
   {
      CalcDrawParams(m_xIncByTime, XcordChangeRate);
      MapDataPoints();
      m_control.Invalidate();
   }
   ...
   public void Chart(Double[] data, int plotCount, int dataCountPerPlot, double xIncBySec, bool recordEndFlag)
   {
      ...
      CalcDrawParams(xIncBySec,XcordChangeRate);
      SaveData(data, m_plotCount, dataCountPerPlot);
      MapDataPoints();
      m_control.Invalidate();
   }

   private void CalcDrawParams(double XIncBySec,double XcordChangeRate)
   {
      ...
      lock (LockObject)
      {
         ...
         Double XcoordinateDivBase = m_size.Width * XIncBySec * 100.0* XcordChangeRate;//ms
      ...
      for (int index = 0; index < drawPoint; ++index)
      {
         for (int i = 0; i < m_plotCount; i++)
         {
            m_dataPointBuffer[i][index].Y = (float)Math.Ceiling(YCordDividedRate *
               (m_YCordRangeMax - AmplificationFactor * m_drawDataBuffer[(int)(m_plotCount * (index + m_mapDataIndexPerPlot + m_shiftCount) + i)]));
